...
public class StartGame : NetworkBehaviour, IWifiInteractable
{
    ...
    public void Interact(CharacterController player)
    {
       ...
       NetworkPlayer nPlayer = player.gameObject.GetComponent<NetworkPlayer>();
        _lookSensitivity = nPlayer.lookSensitivity; // keep track of the original look sensitivity to reset it later
        nPlayer.lookSensitivity = 0;
    ...
    private void Quit()
    {
        ...
        _player.gameObject.GetComponent<NetworkPlayer>().lookSensitivity = _lookSensitivity;
